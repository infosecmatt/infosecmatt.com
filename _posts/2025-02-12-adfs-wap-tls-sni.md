...
title: "AD FS Web Application Proxy: Resolve TCP reset errors connected to wildcard certificate usage"
description: "A customer was having issues with external authentication which I later discovered to be due to how AD FS 2016 handles wildcard certificates. Specifically, attempts to access AD FS from outside of the network were met with an endless stream of TCP resets. This post describes how to resolve that issue by creating a fallback binding for instances where the certificate name does not exactly match the service name."
keywords: "adfs, wap, tls, certificates, troubleshooting, wildcard, authentication, powershell"
categories: itstuff
blogpost_title: "AD FS WAP: Resolve TCP reset errors connected to wildcard certificate usage"
date: 2025-02-12
sitemap:
  lastmod: 2025-02-12
  priority: 0.3
...
![Meme of Patrick Starr (the ADFS WAP server) rejecting the wildcard certificate despite being included in the certificate](/img/posts/adfs-wap-tls-sni/patrick_meme_adfs.png){:class="img-responsive"}
